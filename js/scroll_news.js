(function($) {
	$.fn.scrollfn = function(Parameter, ImgBoxId) {
	  ...
	  if(Parameter.type==1){
		  ImgBox.children(Parameter.ImgBoxChild).css({"opacity":0})
		  ImgBox.children(Parameter.ImgBoxChild).eq(0).css({"opacity":1})
		}
		...
		function btnStart(num) {
			...
			//处理type=1 默认为渐隐渐出的效果 前提是CSS 样式中li 必须是定位的效果
			    ...
			    if(ImgBox.children(Parameter.ImgBoxChild).filter(":visible").index()==i){
				   ...
				   ImgBox.children(Parameter.ImgBoxChild).eq(i).css({"z-index":zindex}).show().animate({"opacity":1},500,function(){
 
					   ImgBox.children(Parameter.ImgBoxChild).not(this).animate({"opacity":0},500).hide()
...
//			}
		...
		function  MultipleBtnfn(){
			...
			leftBtn.click(function() {
				...
				if (thisi < 0) {
					thisi = Math.ceil(ImgBox.children(Parameter.ImgBoxChild).length/Parameter.BnanerGeshu)-Parameter.deomShownum;
				}
				btnStart(thisi);
			})
			}
